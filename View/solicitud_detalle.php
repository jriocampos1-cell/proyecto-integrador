...
try {
    ...
    $stmt = $pdo->prepare('SELECT s.nSolicitudID, u.cNombre AS cUsuario, s.eEstado, s.cMotivoRechazo, s.dFecha FROM TSolicitud s LEFT JOIN TUsuarios u ON s.nUsuarioID = u.nUsuarioID WHERE s.nSolicitudID = :id');
    $stmt->execute([':id' => $id]);
    $solicitud = $stmt->fetch();

    $stmt2 = $pdo->prepare('SELECT d.nDetalleSolicitudID, i.cNombre AS cInsumo, d.nCantidad FROM TDetalleSolicitud d JOIN TInsumos i ON d.nInsumoID = i.nInsumoID WHERE d.nSolicitudID = :id');
    $stmt2->execute([':id' => $id]);
